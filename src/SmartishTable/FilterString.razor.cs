...
public partial class FilterString<SmartishTItem> : INotifyPropertyChanged, IFilter<SmartishTItem>, IDisposable
{
  ...
  /// <summary>
  /// Is Case Sensitive bool.  Default is false
  /// </summary>
  [Parameter]
  ...
  private string[] refreshTableOnParameterChanged = new[] { nameof(Operator), nameof(IsCaseSensitive) };
  ...
  public async void RaisePropertyChange(string propertyname)
  {
    ...
    if (refreshTableOnParameterChanged.Contains(propertyname) && Root != null)
      await Root.Refresh();
  }
